 ...
 */
    static drawLineAA(canvas, x1, y1, x2, y2, r, g, b, a = 255) {
        ...
        if (steep) {
            AADrawer._setPixelAA(canvas, yPx1, xPx1, r, g, b, a * (1 - (yEnd - yPx1)) * xGap);
            AADrawer._setPixelAA(canvas, yPx1 + 1, xPx1, r, g, b, a * (yEnd - yPx1) * xGap);
        } else {
            AADrawer._setPixelAA(canvas, xPx1, yPx1, r, g, b, a * (1 - (yEnd - yPx1)) * xGap);
            AADrawer._setPixelAA(canvas, xPx1, yPx1 + 1, r, g, b, a * (yEnd - yPx1) * xGap);
        }
        pixelsDrawn += 2;
        ...
        if (steep) {
            AADrawer._setPixelAA(canvas, yPx2, xPx2, r, g, b, a * (1 - (yEnd - yPx2)) * xGap);
            AADrawer._setPixelAA(canvas, yPx2 + 1, xPx2, r, g, b, a * (yEnd - yPx2) * xGap);
        } else {
            AADrawer._setPixelAA(canvas, xPx2, yPx2, r, g, b, a * (1 - (yEnd - yPx2)) * xGap);
            AADrawer._setPixelAA(canvas, xPx2, yPx2 + 1, r, g, b, a * (yEnd - yPx2) * xGap);
        }
        pixelsDrawn += 2;
        ...
        canvas.renderer.updateBufferData( // dirty region tracking in c++ 
            canvas.bufferId,
            regionData,
            minX, minY,
 ...
 */
    static drawCircleAA(canvas, cx, cy, radius, r, g, b, a = 255) {
        ...
        // For each pixel in bounding box, calculate coverage
        for (let py = minY; py <= maxY; py++) {
            for (let px = minX; px <= maxX; px++) {
                ...
                // - dist < rad - 0.5: fully inside (100%)
                // - dist > rad + 0.5: fully outside (0%)
                // - Otherwise: partial coverage (linear falloff)
                let coverage;
                ...
                if (dist <= rad - 0.5) {
                    ...
                    // Anti-aliased edge: linear falloff over 1 pixel width
        ...
        const regionHeight = maxY - minY + 1;
        const regionData = AADrawer._extractRegion(
            canvas.data, canvas.width, minX, minY, regionWidth, regionHeight
        );
